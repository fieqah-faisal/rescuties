...
import { Activity, Users, MapPin, AlertTriangle, TrendingUp, Clock, Shield, Flame, Mountain, Waves, Zap, RefreshCw } from 'lucide-react'
...
const Dashboard = () => {
  ...
  const [s3Data, setS3Data] = useState<TwitterDisasterData[]>([])
  const [isLoadingS3, setIsLoadingS3] = useState(false)
  const [lastS3Update, setLastS3Update] = useState<Date | null>(null)
  ...
  // Mock data as fallback when S3 is empty
  const mockAlerts = [
    {
      type: 'Flood',
  ...
  const loadS3Data = async () => {
    setIsLoadingS3(true)
    try {
      const data = await s3Service.getLatestDisasterData()
      setS3Data(data)
      setLastS3Update(new Date())
      
      if (data.length > 0) {
        // Convert S3 data to alert format
        const s3Alerts = data.map(item => ({
          type: item.disaster_type,
          location: item.location?.place_name || 'Unknown Location',
          coordinates: item.location ? { lat: item.location.lat, lng: item.location.lng } : { lat: 4.2105, lng: 108.9758 },
          coordinatesText: item.location ? `${item.location.lat.toFixed(4)}° N, ${item.location.lng.toFixed(4)}° E` : 'Coordinates unavailable',
          timeDetected: new Date(item.created_at).toLocaleString(),
        ...
        setLiveAlerts(s3Alerts)
      } else {
        // Use mock data when S3 is empty
  ...
  useEffect(() => {
    ...
    // Load S3 data initially
    loadS3Data()

    // Set up polling for S3 data every 30 seconds
    const interval = setInterval(loadS3Data, 30000)
              ...
              <div className="flex items-center space-x-3">
                ...
                {lastS3Update && (
                  <span className="text-xs text-gray-400">
                    Updated: {lastS3Update.toLocaleTimeString()}
                  </span>
                )}
                <button 
                  onClick={loadS3Data}
                ...
                >
                  <RefreshCw className={`h-3 w-3 ${isLoadingS3 ? 'animate-spin' : ''}`} />
                  <span>Refresh</span>
                </button>
              </div>
            </div>
